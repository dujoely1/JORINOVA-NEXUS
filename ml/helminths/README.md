# 🪱 Stool / Urine Helminths (worm eggs / O&P) detector — training

Fine-tunes a **YOLOv8** detector to find and identify **helminth ova and larvae**
(roundworms, tapeworms, flukes) on stool/urine microscopy, and produces `helminths.pt`
for the app's vision service. Each detected egg is mapped to its **disease** via
[`backend/ai_services/helminths_organisms.json`](../../backend/ai_services/helminths_organisms.json).

> Intestinal **protozoa** (Entamoeba, Giardia, Cryptosporidium…) are a **separate**
> detector — see [`ml/protozoa/`](../protozoa/).
...
> should match keys in `helminths_organisms.json`; any match → the app attaches
> the disease + significance automatically.

## How to train
1. Open [`helminths_training_colab.ipynb`](helminths_training_colab.ipynb) in Colab → **T4 GPU**.
2. **Step 4** — Option A (Roboflow, ready) or Option B (Kaggle Chula, converts).
3. **Step 6** fine-tunes; **step 7** saves `helminths.pt` → `backend/models/helminths/`.

## Put it in the app
1. Move `helminths.pt` to **`backend/models/helminths/helminths.pt`**, commit, push.
2. The vision service auto-loads it for `image_type` ∈ {`helminths`, `parasitology`, `stool`, `ova`, `urine_parasite`}.
